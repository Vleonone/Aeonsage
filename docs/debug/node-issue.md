...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AeonSage
---

# Node + tsx "__name is not a function" crash

## Summary
Running AeonSage via Node with `tsx` fails at startup with:

```
[aeonsage] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `aeonsage status` and `gateway:watch` worked.
