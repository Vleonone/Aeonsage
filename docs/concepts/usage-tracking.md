...
- `/usage cost` in chats: local cost summary aggregated from AeonSage session logs.
- CLI: `aeonsage status --usage` prints a full per-provider breakdown.
- CLI: `aeonsage channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
