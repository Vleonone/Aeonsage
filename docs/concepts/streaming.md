...
AeonSage has two separate "streaming�?layers:
...
When block streaming is enabled, AeonSage can **merge consecutive block chunks**
...
- Modes: `off` (default), `natural` (800�?500ms), `custom` (`minMs`/`maxMs`).
...
When draft streaming is active, AeonSage disables block streaming for that reply to avoid double-streaming.
