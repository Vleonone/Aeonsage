...
read_when:
  - "You installed AeonSage but `aeonsage` is "command not found�?
...
AeonSage’s runtime baseline is **Node 22+**.

If you can run `npm install -g aeonsage@latest` but later see `aeonsage: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
