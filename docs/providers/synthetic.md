---
summary: "Use Synthetic's Anthropic-compatible API in AeonSage"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. AeonSage registers it as the
...
```bash
aeonsage onboard --auth-choice synthetic-api-key
```
...
Note: AeonSage's Anthropic client appends `/v1` to the base URL, so use
