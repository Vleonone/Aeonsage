---
summary: "Use Venice AI privacy-focused models in AeonSage"
read_when:
  - You want privacy-focused inference in AeonSage
...
```bash
aeonsage onboard --auth-choice venice-api-key
```
...
```bash
aeonsage chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, AeonSage shows all available Venice models. Pick based on your needs:
...
```bash
aeonsage models set venice/claude-opus-45
aeonsage models set venice/llama-3.3-70b
```

List all available models:

```bash
aeonsage models list | grep venice
```

## Configure via `aeonsage configure`

1. Run `aeonsage configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AeonSage automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
aeonsage chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
aeonsage chat --model venice/claude-opus-45

# Use uncensored model
aeonsage chat --model venice/venice-uncensored

# Use vision model with image
aeonsage chat --model venice/qwen3-vl-235b-a22b

# Use coding model
aeonsage chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `aeonsage models list` to see currently available models. Some models may be temporarily offline.
