---
summary: "Model providers (LLMs) supported by AeonSage"
read_when:
...
AeonSage can use many LLM providers. Pick a provider, authenticate, then set the
...
1) Authenticate with the provider (usually via `aeonsage onboard`).
...
- [Google Gemini (API key)](/providers/google)
