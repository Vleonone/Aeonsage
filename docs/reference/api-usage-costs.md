...
AeonSage features that can generate provider usage or paid API calls.
...
- `aeonsage status --usage` and `aeonsage channels list` show provider **usage windows**
...
AeonSage can pick up credentials from:
...
- `aeonsage status --usage`
- `aeonsage models status --json`
...
### 8) Model scan / probe
`aeonsage models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
