...
- Jobs persist under `~/.aeonsage/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.aeonsage/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.aeonsage/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
aeonsage cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
aeonsage cron edit <jobId> --agent ops
aeonsage cron edit <jobId> --clear-agent
```
```

Manual run (debug):
```bash
aeonsage cron run <jobId> --force
```

Edit an existing job (patch fields):
```bash
aeonsage cron edit <jobId> \
...
Run history:
```bash
aeonsage cron runs --id <jobId> --limit 50
```

Immediate system event without creating a job:
```bash
aeonsage system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`aeonsage system event`](/cli/system).

## Troubleshooting

### "Nothing runs�?- Check cron is enabled: `cron.enabled` and `AEONSAGE_SKIP_CRON`.
