---
summary: "Agent tool surface for AeonSage (browser, canvas, nodes, message, cron) replacing legacy `aeonsage-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `aeonsage-*` skills
---

# Tools (AeonSage)

AeonSage exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `aeonsage-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `aeonsage.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, AeonSage logs a warning and ignores the allowlist so core tools stay available.
...
- [Workflows (Lobster)](/tools/lobster): typed workflow runtime with resumable approvals (requires the Lobster CLI on the gateway host).
...
- `host=node` can target a macOS companion app or a headless node host (`aeonsage node run`).
...
Core parameters:
- `query` (required)
- `count` (1�?0; default from `tools.web.search.maxResults`)

Notes:
- Requires a Brave API key (recommended: `aeonsage configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated aeonsage browser.
...
- Quick smoke: `aeonsage nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `aeonsage gateway` restart in-place)
...
- `sessions_send` runs a reply‑back ping‑pong (reply `REPLY_SKIP` to stop; max turns via `session.agentToAgent.maxPingPongTurns`, 0�?).
