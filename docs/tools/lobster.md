---
title: Workflows
summary: "Typed workflow runtime for AeonSage with resumable approval gates."
description: Typed workflow runtime for AeonSage �?composable pipelines with approval gates.
read_when:
...
Workflows (the `lobster` tool) is a workflow shell that lets AeonSage run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: AeonSage runs one Lobster tool call and gets a structured result.
...
AeonSage launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | aeonsage.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
aeonsage.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In AeonSage tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the AeonSage Gateway (see the [Lobster repo](https://github.com/aeonsage/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), AeonSage keeps core tools enabled. To restrict core
...
�?aeonsage calls gmail.list
�?LLM summarizes
�?User: "draft replies to #2 and #5"
�?LLM drafts
�?User: "send #2"
�?aeonsage calls gmail.send
...
- **No secrets** �?Lobster doesn't manage OAuth; it calls aeonsage tools that do.
