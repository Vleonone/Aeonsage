...
Matrix is an open, decentralized messaging protocol. AeonSage connects as a Matrix **user**
...
```bash
aeonsage plugins install @aeonsage/matrix
```

Local checkout (when running from a git repo):

```bash
aeonsage plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `aeonsage plugins install @aeonsage/matrix`
   - From a local checkout: `aeonsage plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AeonSage calls the same
     login endpoint, stores the access token in `~/.aeonsage/credentials/matrix/credentials.json`,
...
- On first connection, AeonSage requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AeonSage logs a warning.
...
`~/.aeonsage/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `aeonsage pairing list matrix`
  - `aeonsage pairing approve matrix <CODE>`
...
- On startup, AeonSage resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
