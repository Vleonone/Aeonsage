...
Note: WhatsApp is a built-in channel, not a plugin. Do not add it under `plugins.entries`.

## Quick setup (beginner)

![WhatsApp QR Linking Protocol](../assets/bridges/whatsapp_qr.png)

1) Use a **separate phone number** if possible (recommended).
2) Configure WhatsApp in `~/.aeonsage/aeonsage.json`.
3) Run `aeonsage channels login` to scan the QR code (Linked Devices).
4) Start the gateway.
...
WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run AeonSage on WhatsApp:

### Dedicated number (recommended)
Use a **separate phone number** for AeonSage. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‑Fi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate �?install WhatsApp Business and register the AeonSage number there.
...
Quick fallback: run AeonSage on **your own number**. Message yourself (WhatsApp "Message yourself�? for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
Self-chat replies default to `[{identity.name}]` when set (otherwise `[aeonsage]`)
...
## Why Not Twilio?
- Early AeonSage builds supported Twilio’s WhatsApp Business integration.
...
## Login + credentials
- Login command: `aeonsage channels login` (QR via Linked Devices).
- Multi-account login: `aeonsage channels login --account <id>` (`<id>` = `accountId`).
- Default account (when `--account` is omitted): `default` if present, otherwise the first configured account id (sorted).
- Credentials stored in `~/.aeonsage/credentials/whatsapp/<accountId>/creds.json`.
- Backup copy at `creds.json.bak` (restored on corruption).
- Legacy compatibility: older installs stored Baileys files directly in `~/.aeonsage/credentials/`.
- Logout: `aeonsage channels logout` (or `--account <id>`) deletes WhatsApp auth state (but keeps shared `oauth.json`).
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `aeonsage pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run AeonSage on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will AeonSage message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. AeonSage only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:
- First DM from a new sender returns a short code (message is not processed).
- Approve with: `aeonsage pairing approve whatsapp <code>` (list with `aeonsage pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different AeonSages on one WhatsApp number?**  
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `aeonsage message send --media <mp4> --gif-playback`
    - Gateway: `send` params include `gifPlayback: true`

## Voice notes (PTT audio)
WhatsApp sends audio as **voice notes** (PTT bubble).
- Best results: OGG/Opus. AeonSage rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Log file: `/tmp/aeonsage/aeonsage-YYYY-MM-DD.log` (configurable).
...
- Fix: run `aeonsage channels login` on the gateway host and scan the QR (WhatsApp �?Settings �?Linked Devices).

**Linked but disconnected / reconnect loop**
- Symptom: `channels status` shows `running, disconnected` or warns "Linked but disconnected�?
- Fix: `aeonsage doctor` (or restart the gateway). If it persists, relink via `channels login` and inspect `aeonsage logs --follow`.
