...
```bash
aeonsage plugins install @aeonsage/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
aeonsage plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "AeonSage" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `aeonsage pairing list nextcloud-talk`
  - `aeonsage pairing approve nextcloud-talk <CODE>`
