...
read_when:
  - Setting up Twitch chat integration for AeonSage
---
# Twitch (plugin)

Twitch chat support via IRC connection. AeonSage connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
aeonsage plugins install @aeonsage/twitch
```

Local checkout (when running from a git repo):

```bash
aeonsage plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `AEONSAGE_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "aeonsage",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use AEONSAGE_TWITCH_ACCESS_TOKEN env var)
...
```bash
AEONSAGE_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
