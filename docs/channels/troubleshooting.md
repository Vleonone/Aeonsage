...
## Plugin not found (Telegram/WhatsApp)
- If you see `plugins.entries.telegram: plugin not found` or `plugins.entries.whatsapp: plugin not found`, remove those entries from config. Telegram and WhatsApp are built-in channels and should only be configured under `channels.telegram` / `channels.whatsapp`.
