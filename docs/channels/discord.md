...
![Discord Fusion Authorization](../assets/bridges/discord_setup.png)
...
## Goals
- Talk to AeonSage via Discord DMs or guild channels.
...
3. Configure AeonSage with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `aeonsage pairing approve discord <code>`.
...
This is the "Discord Developer Portal�?setup for running AeonSage in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents AeonSage needs
...
Discord uses numeric ids everywhere; AeonSage config prefers ids.
...
### Troubleshooting
- First: run `aeonsage doctor` and `aeonsage channels status --probe` (actionable warnings + quick audits).
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        allowFrom: ["123456789012345678", "Vleonone"],
        groupEnabled: false,
        groupChannels: ["aeonsage-dm"]
      },
      guilds: {
        "*": { requireMention: true },
        "123456789012345678": {
          ...
          users: ["987654321098765432", "Vleonone"],
          channels: {
            general: { allow: true },
            help: {
              allow: true,
...
- On startup, AeonSage resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror AeonSage’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; AeonSage enforces allowlists on execution and replies "not authorized�?
...
- If the bot is stuck or rate limited, restart the gateway (`aeonsage gateway --force`) after confirming no other processes own the Discord session.
