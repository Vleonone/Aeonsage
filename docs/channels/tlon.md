...
Tlon is a decentralized messenger built on Urbit. AeonSage connects to your Urbit ship and can
...
```bash
aeonsage plugins install @aeonsage/tlon
```

Local checkout (when running from a git repo):

```bash
aeonsage plugins install ./extensions/tlon
```
...
Use these with `aeonsage message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AeonSage replies in-thread.
