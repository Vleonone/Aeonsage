...
2) Install `imsg`:
   - `brew install Vleonone/tap/imsg`
3) Configure AeonSage with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for AeonSage + `imsg` (Messages DB access).
...
2) Create a macOS user (example: `aeonsagehome`) and sign into it.
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. AeonSage only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. AeonSage can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, AeonSage attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
Architecture:
```
┌──────────────────────────────�?         SSH (imsg rpc)          ┌──────────────────────────�?�?Gateway host (Linux/VM)      │──────────────────────────────────▶│ Mac with Messages + imsg �?�?- aeonsage gateway           �?         SCP (attachments)        �?- Messages signed in     �?�?- channels.imessage.cliPath  │◀──────────────────────────────────�?- Remote Login enabled   �?└──────────────────────────────�?                                  └──────────────────────────�?              �?              �?Tailscale tailnet (hostname or 100.x.y.z)
              �?        user@gateway-host
```
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.aeonsage/scripts/imsg-ssh",
...
Example wrapper (`~/.aeonsage/scripts/imsg-ssh`):
```bash
#!/usr/bin/env bash
...
Multi-account support: use `channels.imessage.accounts` with per-account config and optional `name`. See [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) for the shared pattern. Don't commit `~/.aeonsage/aeonsage.json` (it often contains tokens).
...
- Approve via:
  - `aeonsage pairing list imessage`
  - `aeonsage pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, AeonSage treats that thread as a "group�?for:
