---
summary: "Research notes: offline memory system for AeonSage workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/aeonsage) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep AeonSage integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: AeonSage-style workspace (`agents.defaults.workspace`, default `~/aeonsage`) where "memory" is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/aeonsage` as canonical human-readable memory.
...
- `memory.md` remains "small + core-ish�? the things you want AeonSage to see every session.
...
- at end of day (or during), add a `## Retain` section with 2�? bullets that are:
...
- **temporal**: "what happened around Nov 27�?/ "since last week�?- **opinion**: "what does Peter prefer?�?(with confidence + evidence)
...
Recommendation: **deep integration in AeonSage**, but keep a separable core library.

### Why integrate into AeonSage?
- AeonSage already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `aeonsage memory recall "�? --k 25 --since 30d`
  - `aeonsage memory reflect --since 7d`
