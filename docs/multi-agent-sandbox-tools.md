...
For debugging "why is this blocked?�? see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `aeonsage sandbox explain`.
...
Legacy `agent.*` configs are migrated by `aeonsage doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=aeonsage.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${AEONSAGE_STATE_DIR:-$HOME/.aeonsage}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
