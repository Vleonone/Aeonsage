...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when AeonSage would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When AeonSage would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/aeonsage-alfred/
...
Workspace: /Users/pascal/aeonsage-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.aeonsage/logs/gateway.log | grep broadcast
```
