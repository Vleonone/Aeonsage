...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.aeonsage/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.aeonsage/node.json` (node id + pairing token).
...
Path: `~/.aeonsage/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.aeonsage/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.aeonsage/exec-approvals.sock` (0600).
