...
Delivery:
- Publish as `@aeonsage/plugin-sdk` (or export from core under `aeonsage/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `AeonSagePluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AeonSageConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AeonSageConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AeonSageConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: AeonSageConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AeonSageConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AeonSageConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@aeonsage/plugin-sdk`.
- Add `api.runtime` to `AeonSagePluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `aeonsageRuntime: ">=2026.2.0"`).
