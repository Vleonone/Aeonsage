...
- Replaces the legacy `gateway` command. CLI entry point: `aeonsage gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
aeonsage gateway --port 18789
# for full debug/trace logs in stdio:
aeonsage gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
aeonsage gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.aeonsage/aeonsage.json` (or `AEONSAGE_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__aeonsage__/canvas/` from `~/aeonsage/canvas`. Disable with `canvasHost.enabled=false` or `AEONSAGE_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `AEONSAGE_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `AEONSAGE_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.aeonsage.<profile>`
- Linux: `aeonsage-gateway-<profile>.service`
- Windows: `AeonSage Gateway (<profile>)`

Install metadata is embedded in the service config:
- `AEONSAGE_SERVICE_MARKER=aeonsage`
- `AEONSAGE_SERVICE_KIND=gateway`
- `AEONSAGE_SERVICE_VERSION=<version>`
...
```bash
aeonsage --dev setup
aeonsage --dev gateway --allow-unconfigured
...
- `AEONSAGE_STATE_DIR=~/.aeonsage-dev`
- `AEONSAGE_CONFIG_PATH=~/.aeonsage-dev/aeonsage.json`
- `AEONSAGE_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/aeonsage-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `AEONSAGE_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `AEONSAGE_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `AEONSAGE_CONFIG_PATH`
- unique `AEONSAGE_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
aeonsage --profile main gateway install
aeonsage --profile rescue gateway install
```

Example:
```bash
AEONSAGE_CONFIG_PATH=~/.aeonsage/a.json AEONSAGE_STATE_DIR=~/.aeonsage-a aeonsage gateway --port 19001
AEONSAGE_CONFIG_PATH=~/.aeonsage/b.json AEONSAGE_STATE_DIR=~/.aeonsage-b aeonsage gateway --port 19002
```
...
## Methods (initial set)
- `health` �?full health snapshot (same shape as `aeonsage health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `aeonsage`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `aeonsage gateway install` writes `~/Library/LaunchAgents/com.aeonsage.gateway.plist`
    (or `com.aeonsage.<profile>.plist`).
  - `aeonsage doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are AeonSage profile services.
  ...
  - Cleanup: `aeonsage gateway uninstall` (current service) and `aeonsage doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `aeonsage gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- AeonSage.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.aeonsage.gateway` (or `com.aeonsage.<profile>`).
- To stop it cleanly, use `aeonsage gateway stop` (or `launchctl bootout gui/$UID/com.aeonsage.gateway`).
- To restart, use `aeonsage gateway restart` (or `launchctl kickstart -k gui/$UID/com.aeonsage.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `aeonsage gateway install` first.
  - Replace the label with `com.aeonsage.<profile>` when running a named profile.

## Supervision (systemd user unit)
AeonSage installs a **systemd user service** by default on Linux/WSL2. We
...
`aeonsage gateway install` writes the user unit. `aeonsage doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/aeonsage-gateway[-<profile>].service`:
```
[Unit]
Description=AeonSage Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/aeonsage gateway --port 18789
Restart=always
RestartSec=5
Environment=AEONSAGE_GATEWAY_TOKEN=
...
```
systemctl --user enable --now aeonsage-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/aeonsage-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now aeonsage-gateway[-<profile>].service
```
...
## CLI helpers
- `aeonsage gateway health|status` �?request health/status over the Gateway WS.
- `aeonsage message send --target <num> --message "hi" [--media ...]` �?send via Gateway (idempotent for WhatsApp).
- `aeonsage agent --message "hi" --to <num>` �?run an agent turn (waits for final by default).
- `aeonsage gateway call <method> --params '{"k":"v"}'` �?raw method invoker for debugging.
- `aeonsage gateway stop|restart` �?stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `aeonsage gateway` and the legacy TCP control port.
