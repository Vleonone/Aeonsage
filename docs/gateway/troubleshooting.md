---
summary: "Quick troubleshooting guide for common AeonSage failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When AeonSage misbehaves, here's how to fix it.
...
|---|---|---|
| `aeonsage status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `aeonsage status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `aeonsage status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When "configured�?doesn’t mean "working�?|
| `aeonsage gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `aeonsage channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `aeonsage gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service "looks loaded�?but nothing runs |
| `aeonsage logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `aeonsage status --all` (it redacts tokens). If you paste `aeonsage status`, consider setting `AEONSAGE_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  aeonsage models auth setup-token --provider anthropic
  ```
...
```bash
aeonsage models auth paste-token --provider anthropic
aeonsage models status
```
...
**Logs:**
- Preferred: `aeonsage logs --follow`
- File logs (always): `/tmp/aeonsage/aeonsage-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$AEONSAGE_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u aeonsage-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "AeonSage Gateway (<profile>)" /V /FO LIST`
...
- Or set it directly:
  ```bash
  aeonsage config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  aeonsage config set gateway.mode remote
  aeonsage config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `aeonsage setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.aeonsage/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `aeonsage doctor`
...
- then run `aeonsage sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `aeonsage gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `aeonsage gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `aeonsage gateway status` and `aeonsage doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `AEONSAGE_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `aeonsage gateway status` shows a config mismatch**
...
- Fix: rerun `aeonsage gateway install --force` from the same `--profile` / `AEONSAGE_STATE_DIR` you want the service to use.

**If `aeonsage gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `aeonsage doctor` to update it (or `aeonsage gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `AEONSAGE_GATEWAY_TOKEN`) and restart the service.

**If `aeonsage gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/aeonsage` on disk.
...
Symptoms: `pwd` or file tools show `~/.aeonsage/sandboxes/...` even though you
...
AeonSage intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `aeonsage models list` or
  `aeonsage models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${AEONSAGE_CONFIG_PATH:-$HOME/.aeonsage/aeonsage.json}"
```
...
tail -f "$(ls -t /tmp/aeonsage/aeonsage-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
aeonsage logs --follow | grep "pairing request"
```
...
- �?`@aeonsage` + image
- �?`@aeonsage check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.aeonsage/agents/<agentId>/sessions/
```
...
aeonsage status --deep

# View recent connection events
aeonsage logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
aeonsage channels logout
trash "${AEONSAGE_STATE_DIR:-$HOME/.aeonsage}/credentials" # if logout can't cleanly remove everything
aeonsage channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/aeonsage/aeonsage-*.log | head -1)" | tail -20
```

### High Memory Usage

AeonSage keeps conversation history in memory.
...
AeonSage now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `aeonsage doctor` reports every invalid entry.
- `aeonsage doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `aeonsage logs`, `aeonsage health`, `aeonsage status`, `aeonsage gateway status`, and `aeonsage gateway probe` still run even if the config is invalid.

### "All models failed�?�?what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/aeonsage/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `aeonsage models status` (CLI).
...
```bash
curl -fsSL https://raw.githubusercontent.com/Vleonone/AeonSage-Silicon-Intelligence/main/scripts/quickstart.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `aeonsage channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. AeonSage scrubs/normalizes tool
...
```bash
tccutil reset All com.aeonsage.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/aeonsage/aeonsage/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/com.aeonsage.gateway (replace with com.aeonsage.<profile> if needed)
```
...
Ensure the global `aeonsage` CLI is installed and matches the app version:
```bash
aeonsage --version
npm install -g aeonsage@<version>
```
...
#   ${AEONSAGE_CONFIG_PATH:-$HOME/.aeonsage/aeonsage.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
aeonsage gateway --verbose
aeonsage channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/aeonsage/aeonsage-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$AEONSAGE_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.aeonsage/logs/...`; profiles use `~/.aeonsage-<profile>/logs/...`)<br />Linux: `journalctl --user -u aeonsage-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "AeonSage Gateway (<profile>)" /V /FO LIST` |
| Session files | `$AEONSAGE_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$AEONSAGE_STATE_DIR/media/` |
| Credentials | `$AEONSAGE_STATE_DIR/credentials/` |
...
tail -20 /tmp/aeonsage/aeonsage-*.log
```
...
trash "${AEONSAGE_STATE_DIR:-$HOME/.aeonsage}"
aeonsage channels login         # re-pair WhatsApp
aeonsage gateway restart           # or: aeonsage gateway
```
...
1. Check logs first: `/tmp/aeonsage/` (default: `aeonsage-YYYY-MM-DD.log`, or your configured `logging.file`)
