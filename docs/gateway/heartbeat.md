...
- During heartbeat runs, AeonSage treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `aeonsage sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AeonSage skips the heartbeat run entirely (no model call).
...
headers like `# Heading`), AeonSage skips the heartbeat run to save API calls.
...
- "Update `HEARTBEAT.md` to add a daily calendar check.�?- "Rewrite `HEARTBEAT.md` so it’s shorter and focused on inbox follow-ups.�?
...
```bash
aeonsage system event --text "Check for urgent follow-ups" --mode now
```
