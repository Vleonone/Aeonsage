---
summary: "All configuration options for ~/.aeonsage/aeonsage.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

AeonSage reads an optional **JSON5** config from `~/.aeonsage/aeonsage.json` (comments + trailing commas allowed).

If the file is missing, AeonSage uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/aeonsage`). You usually only need a config to:
...
AeonSage only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `aeonsage doctor`, `aeonsage logs`, `aeonsage health`, `aeonsage status`, `aeonsage service`, `aeonsage help`).
- Run `aeonsage doctor` to see the exact issues.
- Run `aeonsage doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `aeonsage config set`. Keep a backup of `~/.aeonsage/aeonsage.json`.
...
```bash
aeonsage gateway call config.get --params '{}' # capture payload.hash
aeonsage gateway call config.apply --params '{
  "raw": "{\\n  agents: { defaults: { workspace: \\"~/aeonsage\\" } }\\n}\\n",
...
```bash
aeonsage gateway call config.get --params '{}' # capture payload.hash
aeonsage gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/aeonsage" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@aeonsage", "reisponde"] }
      }
    ]
  },
  channels: {
...
```json5
// ~/.aeonsage/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [
    { id: "main", workspace: "~/aeonsage" }
  ]
}
```
...
{ "$include": "/etc/aeonsage/base.json5" } // absolute
...
```json5
// ~/.aeonsage/clients/mueller/agents.json5
[
...
```json5
// ~/.aeonsage/clients/mueller/broadcast.json5
{
...
AeonSage reads env vars from the parent process (shell, launchd/systemd, CI, etc.).

Additionally, it loads:
- `.env` from the current working directory (if present)
- a global fallback `.env` from `~/.aeonsage/.env` (aka `$AEONSAGE_STATE_DIR/.env`)
...
Opt-in convenience: if enabled and none of the expected keys are set yet, AeonSage runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `AEONSAGE_LOAD_SHELL_ENV=1`
- `AEONSAGE_SHELL_ENV_TIMEOUT_MS=15000`
...
AeonSage stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.aeonsage/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.aeonsage/credentials/oauth.json` (or `$AEONSAGE_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.aeonsage/agent/*` (migrated by `aeonsage doctor` into `~/.aeonsage/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `AEONSAGE_OAUTH_DIR`
- Agent dir (default agent root override): `AEONSAGE_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, AeonSage imports `oauth.json` entries into `auth-profiles.json`.
...
If set, AeonSage derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/aeonsage/aeonsage-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/aeonsage/aeonsage.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/aeonsage/aeonsage.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
...
Pairing approvals:
- `aeonsage pairing list whatsapp`
- `aeonsage pairing approve whatsapp <code>`
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.aeonsage/credentials/whatsapp/biz
          // authDir: "~/.aeonsage/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `aeonsage doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@aeonsage", "aeonsage", "aeonsage"] } }
    ]
  }
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@aeonsage"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/aeonsage-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.aeonsage/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/aeonsage-home" },
      { id: "work", workspace: "~/aeonsage-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `aeonsage.json`).
...
AeonSage starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        allowFrom: ["1234567890", "Vleonone"], // optional DM allowlist ("open" requires ["*"])
        groupEnabled: false,                 // enable group DMs
        groupChannels: ["aeonsage-dm"]          // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {               // guild id (preferred) or slug
          slug: "friends-of-aeonsage",
...
AeonSage starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `aeonsage plugins install @aeonsage/mattermost` (or `./extensions/mattermost` from a git checkout).
...
AeonSage starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
AeonSage spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, AeonSage
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/aeonsage" } }
}
```
...
When a file exceeds this limit, AeonSage logs a warning and injects a truncated
...
message envelopes). If unset, AeonSage uses the host timezone at runtime.
...
`[aeonsage]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[aeonsage]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[aeonsage]"`, AeonSage will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, AeonSage generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.aeonsage/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"AeonSage"`).
...
AeonSage also ships a few built-in alias shorthands. Defaults only apply when the model
...
- `agents.defaults.blockStreamingChunk`: soft chunking for streamed blocks. Defaults to
  800�?200 chars, prefers paragraph breaks (`\n\n`), then newlines, then sentences.
  Example:
  ```json5
  {
...
- `agents.defaults.humanDelay`: randomized pause between **block replies** after the first.
  Modes: `off` (default), `natural` (800�?500ms), `custom` (use `minMs`/`maxMs`).
...
If you omit the provider, AeonSage currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `aeonsage configure --section web`, or use `BRAVE_API_KEY` env var)
- `tools.web.search.maxResults` (1�?0, default 5)
...
- `group:nodes`: `nodes`
- `group:aeonsage`: all built-in AeonSage tools (excludes provider plugins)
  ...
  tools: {
    elevated: {
      ...
      allowFrom: {
        ...
        discord: ["Vleonone", "1234567890123"]
      }
    }
  }
}
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.aeonsage/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.aeonsage/sandboxes",
        docker: {
          image: "aeonsage-sandbox:bookworm-slim",
          containerPrefix: "aeonsage-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "aeonsage-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "aeonsage-sandbox-browser:bookworm-slim",
          containerPrefix: "aeonsage-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
AeonSage uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.aeonsage/agents/<agentId>/agent/models.json` or by defining the same schema inside your
AeonSage config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, AeonSage writes/merges a `models.json` into
`~/.aeonsage/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. AeonSage uses
...
- Shortcut: `aeonsage onboard --auth-choice opencode-zen`.
...
Shortcut: `aeonsage onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `aeonsage onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `aeonsage onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `aeonsage onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `aeonsage onboard --auth-choice minimax-api`.
...
- Override the agent config root with `AEONSAGE_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.aeonsage/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.aeonsage/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.aeonsage/agents/{agentId}/sessions/sessions.json",
    // Direct chats collapse to agent:<agentId>:<mainKey> (default: "main").
    mainKey: "main",
    agentToAgent: {
      // Max ping-pong reply turns between requester/target (0�?).
      maxPingPongTurns: 5
    },
    sendPolicy: {
      rules: [
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, AeonSage stays in idle-only mode for backward compatibility.
- `heartbeatIdleMinutes`: optional idle override for heartbeat checks (daily reset still applies when enabled).
- `agentToAgent.maxPingPongTurns`: max reply-back turns between requester/target (0�?, default 5).
...
overrides. Applies to **bundled** skills and `~/.aeonsage/skills` (workspace skills
...
from `~/.aeonsage/extensions`, `<workspace>/.aeonsage/extensions`, plus any
...
### `browser` (aeonsage-managed browser)

AeonSage can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for aeonsage and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (AeonSage.app menubar, or `aeonsage gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      aeonsage: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/aeonsage" }
...
### `gateway.securityMode` (Security mode preset)

Use `gateway.securityMode` to set a safety vs efficiency preset for tool defaults.
Explicit `tools.*` values always win.

Mapping:
- `secure`: `tools.profile = minimal`, `tools.exec.security = allowlist`, `tools.exec.ask = on-miss`
- `efficient`: `tools.profile = full`, `tools.exec.security = full`, `tools.exec.ask = off`

```json5
{
  gateway: {
    securityMode: "secure" // or "efficient"
  }
}
```
...
- Examples: `"/ui"`, `"/aeonsage"`, `"/apps/aeonsage"`.
...
- When a connection comes from one of these IPs, AeonSage uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `aeonsage gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `AEONSAGE_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `AEONSAGE_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. AeonSage
...
macOS app behavior:
- AeonSage.app watches `~/.aeonsage/aeonsage.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.aeonsage/aeonsage.json` (or `AEONSAGE_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.aeonsage/aeonsage.json` (or `AEONSAGE_CONFIG_PATH`)
...
- `AEONSAGE_CONFIG_PATH` (per-instance config)
- `AEONSAGE_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `aeonsage --dev …` �?uses `~/.aeonsage-dev` + shifts ports from base `19001`
- `aeonsage --profile <name> …` �?uses `~/.aeonsage-<name>` (port via config/env/flags)
...
Example:
```bash
AEONSAGE_CONFIG_PATH=~/.aeonsage/a.json \
AEONSAGE_STATE_DIR=~/.aeonsage-a \
aeonsage gateway --port 19001
```
...
Gmail helper config (used by `aeonsage webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "aeonsage@gmail.com",
...
- Set `AEONSAGE_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, AeonSage defaults `serve.path` to `/` so
...
Default root: `~/aeonsage/canvas`  
Default port: `18793` (chosen to avoid the aeonsage browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__aeonsage/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__aeonsage__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `AEONSAGE_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.aeonsage/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.aeonsage/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.aeonsage/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_aeonsage-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_aeonsage-bridge._tcp` under `~/.aeonsage/dns/` using the standard discovery domain `aeonsage.internal.`

To make iOS/Android discover across networks (Vienna �?London), pair this with:
- a DNS server on the gateway host serving `aeonsage.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `aeonsage.internal` via that server
