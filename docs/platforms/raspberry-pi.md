---
summary: "AeonSage on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up AeonSage on a Raspberry Pi
  - Running AeonSage on ARM devices
  - Building a cheap always-on personal AI
---

# AeonSage on Raspberry Pi

## Goal

Run a persistent, always-on AeonSage Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
| **Pi 4** | 1GB | Tight | Possible with swap, minimal config |
...
Most AeonSage features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled aeonsage

# Enable if not
sudo systemctl enable aeonsage

# Start on boot
sudo systemctl start aeonsage
```
...
```bash
# Check logs
journalctl -u aeonsage --no-pager -n 100

# Common fix: rebuild
cd ~/aeonsage  # if using hackable install
npm run build
sudo systemctl restart aeonsage
```
