---
summary: "Run AeonSage Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: AeonSage Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM �?install Node 22 �?install AeonSage  
2) Run `aeonsage onboard --install-daemon`  
...
```bash
ssh exe.dev new --name=aeonsage
```

Then connect:

```bash
ssh aeonsage.exe.xyz
```

Tip: keep this VM **stateful**. AeonSage stores state under `~/.aeonsage/` and `~/aeonsage/`.
...
```bash
ssh -N -L 18789:127.0.0.1:18789 aeonsage.exe.xyz
```
...
```bash
export AEONSAGE_GATEWAY_TOKEN="$(openssl rand -hex 32)"
aeonsage gateway --bind lan --port 8080 --token "$AEONSAGE_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.aeonsage/aeonsage.json`:
...
Notes:
- Non-loopback binds require `gateway.auth.token` (or `AEONSAGE_GATEWAY_TOKEN`).
- `gateway.remote.token` is only for remote CLI calls; it does not enable local auth.

Then point exe.dev’s proxy at `8080` (or whatever port you chose) and open your VM’s HTTPS URL:

```bash
ssh exe.dev share port aeonsage 8080
```

Open:
- `https://aeonsage.exe.xyz/`
...
On Linux, AeonSage uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status aeonsage-gateway[-<profile>].service
```
