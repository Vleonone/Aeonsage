...
read_when:
  - Installing AeonSage on Windows
  - Looking for Windows companion app status
---
# Windows (WSL2)

AeonSage on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
- Remote nodes must point at a **reachable** Gateway URL (not `127.0.0.1`); use
  `aeonsage status --all` to confirm.
