---
summary: "First-run onboarding flow for AeonSage (macOS app)"
read_when:
...
- Writes credentials to `~/.aeonsage/credentials/oauth.json`
...
The app can install the global `aeonsage` CLI via npm/pnpm so terminal
...
On the first agent run, AeonSage bootstraps a workspace (default `~/aeonsage`):
...
```bash
aeonsage webhooks gmail setup --account you@gmail.com
```
...
- `~/.aeonsage/credentials/oauth.json`
