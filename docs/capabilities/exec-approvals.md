...
- **gateway host** â†?`aeonsage` process on the gateway machine
...
`~/.aeonsage/exec-approvals.json` directly.

CLI: `aeonsage approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
