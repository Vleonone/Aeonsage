---
summary: "Chrome extension: let AeonSage drive your existing Chrome tab"
read_when:
...
The AeonSage Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate AeonSage-managed Chrome profile.
...
AeonSage then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
- Enable "Developer mode�?- "Load unpacked�?�?select the directory printed above

4) Pin the extension.

## Updates (no build step)

The extension ships inside the AeonSage release (npm package) as static files. There is no separate "build�?step.

After upgrading AeonSage:
- Re-run `aeonsage browser extension install` to refresh the installed files under your AeonSage state directory.
- Chrome �?`chrome://extensions` �?click "Reload�?on the extension.

## Use it (no extra config)

AeonSage ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want AeonSage to control.
...
- `ON`: attached; AeonSage can drive that tab.
...
Debugging: `aeonsage sandbox explain`
...
`aeonsage browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `aeonsage browser extension install` first to copy the extension to a stable location under your AeonSage state directory.
...
- **This is not isolated** like the dedicated AeonSage-managed profile.
