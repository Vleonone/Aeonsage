...
Deploy AeonSage on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/aeonsage/aeonsage" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run AeonSage:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: AEONSAGE_STATE_DIR
        value: /data/.aeonsage
      - key: AEONSAGE_WORKSPACE_DIR
        value: /data/workspace
      - key: AEONSAGE_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/aeonsage`.
...
If you use the original AeonSage repository, Render will not auto-deploy your AeonSage. To update it, run a manual Blueprint sync from the dashboard.
...
For AeonSage, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any AeonSage host.
