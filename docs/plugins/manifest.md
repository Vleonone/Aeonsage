...
read_when:
  - You are building a AeonSage plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (aeonsage.plugin.json)

Every plugin **must** ship a `aeonsage.plugin.json` file in the **plugin root**.
AeonSage uses this manifest to validate configuration **without executing plugin
