...
function resolveAnthropicDefaultAuthMode(cfg: AeonSageConfig): AnthropicAuthDefaultsMode | null {
...
export function applyMessageDefaults(cfg: AeonSageConfig): AeonSageConfig {
...
export function applyTalkApiKey(config: AeonSageConfig): AeonSageConfig {
...
export function applyModelDefaults(cfg: AeonSageConfig): AeonSageConfig {
  let mutated = false;
  let nextCfg = cfg;
    ...
    for (const [providerId, provider] of Object.entries(providerConfig)) {
      ...
      const nextModels = models.map((model) => {
        ...
        const maxTokens = Math.min(
          isPositiveNumber(raw.maxTokens) ? raw.maxTokens : defaultMaxTokens,
          contextWindow,
        );
...
export function applyAgentDefaults(cfg: AeonSageConfig): AeonSageConfig {
...
export function applyLoggingDefaults(cfg: AeonSageConfig): AeonSageConfig {
...
export function applySecurityModeDefaults(cfg: AeonSageConfig): AeonSageConfig {
  const mode = cfg.gateway?.securityMode;
  if (!mode) return cfg;
  ...
  if (mode === "efficient") {
    if (tools.profile === undefined) {
      nextTools.profile = "full";
      mutated = true;
    }
    if (exec.security === undefined) {
      nextExec.security = "full";
      mutated = true;
      execMutated = true;
    }
    if (exec.ask === undefined) {
...
export function applyContextPruningDefaults(cfg: AeonSageConfig): AeonSageConfig {
...
export function applyCompactionDefaults(cfg: AeonSageConfig): AeonSageConfig {
  const defaults = cfg.agents?.defaults;
  if (!defaults) return cfg;
