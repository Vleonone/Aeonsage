...
import { getDashboardStatusMessages, getCommonMessages, resolveUiLocale } from "../i18n/ui.js";
    ...
    if (fromQuery) {
      logHooks.warn(
        "Hook token provided via query parameter is deprecated for security reasons. " +
          ...
          "Use Authorization: Bearer <token> or X-AeonSage-Token header instead.",
      );
    }
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      if (
        ...
        return;
      // Handle Kali security tool routes (authorized only)
      if (
        await handleKaliHttpRequest(req, res, {
          projectRoot: process.cwd(),
        })
      )
        return;
      if (url.pathname === "/api/status") {
        ...
        const messages = getDashboardStatusMessages(locale);
      ...
      if (canvasHost) {
        if (await handleA2uiHttpRequest(req, res, { auth: resolvedAuth, trustedProxies })) return;
